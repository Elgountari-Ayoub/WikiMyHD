  ...
  <DashboardLayout>
    <div class="container mx-auto p-4" v-if="user">
      <div class="flex flex-col gap-4 w-52 items-center">
        <img v-if="user.photo" class="w-52 h-52 rounded-full mr-auto" :src="getImageUrl(user.photo)"
          alt="">
        <svg v-else fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"
          class="w-52 h-52 rounded-full mr-auto
...
//   //   newPassword.value = user.value.password;
//   // } catch (error) {
//   //   console.log(error);
//   // }
//   user.value = userStore;
...
//   newEmail.value = user.value.email
//   newPassword.value = user.value.password
//   newPhoto.value = user.value.photo
//   newRole.value = user.value.role
//   // newName.value = userStore.name || null
// });
