<template>
  ...
  <div>
    <DashboardLayout>
      ...
      <RouterLink :to="{ name: 'space', params: {id: `${space.id}`} }" class="hover:text-blue-500 text-base  rounded">
        {{ space.title }}
      </RouterLink>
...
const space = ref([]);
const getSpace = onMounted(async () => {
  console.log(spaceStore);
  spaceStore.getSpace(route.params.id);
})

const getCreatorId = (users) => {
  let creatorId = 1;
  spaceStore.users.forEach(user => {
